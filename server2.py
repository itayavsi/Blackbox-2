...
def setup_level5_encrypted_log():
    ...
    log_dir = r"C:\ProgramData\CTF_Challenge\logs"
    log_path = os.path.join(log_dir, "security_log.enc")
...
def start_level4_socket_server():
    global socket_server_running, socket_server_thread
    if not socket_server_running:
        socket_server_thread = threading.Thread(target=level4_socket_server, daemon=True)
